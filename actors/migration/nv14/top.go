...
// XXX balanceTransferMao should become a BalanceTable, maybe?
...
func MigrateStateTree(ctx context.Context, store cbor.IpldStore, actorsRootIn cid.Cid, priorEpoch abi.ChainEpoch, cfg Config, log Logger, cache MigrationCache) (cid.Cid, error) {
	...
	if err != nil || !found {
		return cid.Undef, xerrors.Errorf("Could not get market actor for migration.")
	}
	var marketState market5.State
