...
func (t *WinningPoStVerifyInfo) MarshalCBOR(w io.Writer) error {
	...
	// t.EligibleSectors ([]abi.SectorNumber) (slice)
	if len(t.EligibleSectors) > cbg.MaxLength {
		return xerrors.Errorf("Slice value in field t.EligibleSectors was too long")
	}

	if _, err := w.Write(cbg.CborEncodeMajorType(cbg.MajArray, uint64(len(t.EligibleSectors)))); err != nil {
		return err
	}
	for _, v := range t.EligibleSectors {
		if err := cbg.CborWriteHeader(w, cbg.MajUnsignedInt, uint64(v)); err != nil {
			return err
		}
	}
...
func (t *WinningPoStVerifyInfo) UnmarshalCBOR(r io.Reader) error {
	...
	if extra > cbg.MaxLength {
		return fmt.Errorf("t.EligibleSectors: array too large (%d)", extra)
	}
	...
	if extra > 0 {
		t.EligibleSectors = make([]SectorNumber, extra)
	}
	for i := 0; i < int(extra); i++ {

		maj, val, err := cbg.CborReadHeader(br)
		if err != nil {
			return xerrors.Errorf("failed to read uint64 for t.EligibleSectors slice: %w", err)
		}

		if maj != cbg.MajUnsignedInt {
			return xerrors.Errorf("value read for array t.EligibleSectors was not a uint, instead got %d", maj)
		}

		t.EligibleSectors[i] = SectorNumber(val)
	}
