...
func (a Actor) ReportConsensusFault(rt Runtime, params *ReportConsensusFaultParams) *abi.EmptyValue {
	...
	epochBlockReward := rewardStats.ThisEpochRewardSmoothed.Estimate()
	faultPenalty := ConsensusFaultPenalty(epochBlockReward)
	slasherReward := RewardForConsensusSlashReport(epochBlockReward)
	pledgeDelta := big.Zero()
