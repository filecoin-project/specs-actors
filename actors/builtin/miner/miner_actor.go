...
func (a Actor) Exports() []interface{} {
	return []interface{}{
		...
		28:                        a.ChangeBeneficiaryChange,
		29:                        a.ConfirmBeneficiaryChange,
	}
}
...
func (a Actor) WithdrawBalance(rt Runtime, params *WithdrawBalanceParams) *abi.TokenAmount {
	...
	rt.StateTransaction(&st, func() {
		...
		// and not the worker.
		rt.ValidateImmediateCallerIs(info.Owner, info.BeneficiaryInfo.Addr)
...
type ChangeeBeneficiaryParams struct {
	NewBeneficiary          addr.Address
	NewBeneficialQuota      abi.TokenAmount
	NewBeneficialExpireDate abi.ChainEpoch
}

func checkNewBeneficialParam(rt Runtime, params *ChangeeBeneficiaryParams) {
	if params.NewBeneficialExpireDate <= rt.CurrEpoch() {
...
func checkIncreaseQuota(rt Runtime, curBeneficiary BeneficiaryInfo, params *ChangeeBeneficiaryParams) {
	if params.NewBeneficialQuota.LessThanEqual(curBeneficiary.Quota) {
		rt.Abortf(exitcode.ErrIllegalArgument, "new beneficial quota {} must larger than old one {}", params.NewBeneficialQuota, curBeneficiary.Quota)
	}
	if params.NewBeneficialExpireDate < curBeneficiary.ExpireDate {
		rt.Abortf(exitcode.ErrIllegalArgument, "new beneficial expire date {} must big or equal than old one {}", params.NewBeneficialQuota, curBeneficiary.Quota)
	}
}
...
func (a Actor) ChangeBeneficiaryChange(rt Runtime, params *ChangeeBeneficiaryParams) *abi.EmptyValue {
	...
	rt.StateTransaction(&st, func() {
		...
		if info.BeneficiaryInfo.Addr == info.Owner && newBeneficiary != info.Owner {
			...
			//ChangeBeneficiary command needs to propose a new beneficiary address, at the same time, the pending beneficiaryInfo can be set at the same time
			...
			if info.BeneficiaryInfo.UsedUpOrExpire(rt.CurrEpoch()) {
				...
				if newBeneficiary == info.Owner {
					...
					if newBeneficiary == info.BeneficiaryInfo.Addr {
						...
						//2.3
						//When there is a new beneficiary (not the owner itself), after the owner sent the ChangeBeneficiary command to change the beneficiary and beneficiaryInfo,
						//the new beneficiary should send the same command to confirm, and then it can take effect. In addition, the usedQuota is set to 0 since it's new.
						checkNewBeneficialParam(rt, params)
						info.PendingBeneficiaryInfo = &PendingBeneficiaryChange{
							NewBeneficiary:          newBeneficiary,
...
func (a Actor) ConfirmBeneficiaryChange(rt Runtime, _ *abi.EmptyValue) *abi.EmptyValue {
	var st State
	rt.StateTransaction(&st, func() {
		info := getMinerInfo(rt, &st)

		if info.PendingBeneficiaryInfo == nil {
			rt.Abortf(exitcode.ErrIllegalArgument, "no pending beneficiary change")
		}

		builtin.RequireState(rt, info.PendingBeneficiaryInfo.NewBeneficialExpireDate > rt.CurrEpoch(),
			"pending beneficiary has expire current epoch %d beneficial expire date %d", rt.CurrEpoch(), info.PendingBeneficiaryInfo.NewBeneficialExpireDate)

		if info.BeneficiaryInfo.Addr == info.Owner && info.PendingBeneficiaryInfo.NewBeneficiary != info.Owner {
			// Only the  is allowed to change the newWorker.
			rt.ValidateImmediateCallerIs(info.PendingBeneficiaryInfo.NewBeneficiary)
			info.BeneficiaryInfo = BeneficiaryInfo{
				Addr:       info.PendingBeneficiaryInfo.NewBeneficiary,
				Quota:      info.PendingBeneficiaryInfo.NewBeneficialQuota,
				ExpireDate: info.PendingBeneficiaryInfo.NewBeneficialExpireDate,
				UsedQuota:  big.Zero(),
			}
			info.PendingBeneficiaryInfo = nil
		} else {
			if info.PendingBeneficiaryInfo.NewBeneficiary == info.BeneficiaryInfo.Addr {
				rt.ValidateImmediateCallerIs(info.PendingBeneficiaryInfo.NewBeneficiary)
				info.BeneficiaryInfo.Addr = info.PendingBeneficiaryInfo.NewBeneficiary
				info.BeneficiaryInfo.ExpireDate = info.PendingBeneficiaryInfo.NewBeneficialExpireDate
				info.PendingBeneficiaryInfo = nil
			} else {
				if info.BeneficiaryInfo.UsedUpOrExpire(rt.CurrEpoch()) {
					rt.ValidateImmediateCallerIs(info.PendingBeneficiaryInfo.NewBeneficiary)
					...
					if info.PendingBeneficiaryInfo.OldBeneficiaryConfirmed {
						rt.ValidateImmediateCallerIs(info.PendingBeneficiaryInfo.NewBeneficiary)
						info.BeneficiaryInfo = BeneficiaryInfo{
							Addr:       info.PendingBeneficiaryInfo.NewBeneficiary,
							Quota:      info.PendingBeneficiaryInfo.NewBeneficialQuota,
						...
						rt.ValidateImmediateCallerIs(info.BeneficiaryInfo.Addr)
						info.PendingBeneficiaryInfo.OldBeneficiaryConfirmed = true
					}
				}
			}
		}
