	...
	rt.State().Transaction(&st, func() interface{} {
		...
		if err != nil {
			rt.Abortf(exitcode.ErrIllegalState, "failed to compute partitions sectors at deadline %d, partitions %s: %w",
				deadline.Index, params.Partitions, err)
		}
		...
		if err != nil {
			rt.Abortf(exitcode.ErrIllegalState, "failed to diff bitfields: %s", err)
		}

		empty, err := nonFaults.IsEmpty()
		if err != nil {
			rt.Abortf(exitcode.ErrIllegalState, "failed to check if bitfield was empty: %s", err)
		}
		if empty {
...
func (a Actor) checkPrecommitExpiry(rt Runtime, sectors *abi.BitField, regProof abi.RegisteredProof) {
	...
	rt.State().Transaction(&st, func() interface{} {
		_ = sectors.ForEach(func(i uint64) error {
...
func (a Actor) terminateSectors(rt Runtime, sectorNos *abi.BitField, terminationType power.SectorTermination) {
	...
	rt.State().Transaction(&st, func() interface{} {
		...
		_ = sectorNos.ForEach(func(sectorNo uint64) error {
