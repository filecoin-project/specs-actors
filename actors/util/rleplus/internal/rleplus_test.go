...
import (
	...
	"github.com/filecoin-project/specs-actors/actors/util/rleplus/internal"
)
...
func TestRleplus(t *testing.T) {
	...
	t.Run("Encode", func(t *testing.T) {
		...
		actualBytes, _, err := bitvector.Encode(ints)
	...
	t.Run("Encode allows all runs sizes possible uint64", func(t *testing.T) {
		...
		encodeBytes, _, err := bitvector.Encode(ints)
		assert.NoError(t, err)
	...
	t.Run("Decode", func(t *testing.T) {
		...
		for _, tc := range testCases {
			encoded, _, err := bitvector.Encode(tc)
			assert.NoError(t, err)

			result, err := bitvector.Decode(encoded)
			assert.NoError(t, err)
	...
	t.Run("Decode version check", func(t *testing.T) {
		_, err := bitvector.Decode([]byte{0xff})
	...
	t.Run("Decode returns an error with a bad encoding", func(t *testing.T) {
		...
		_, err := bitvector.Decode([]byte{0xe0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff})
	...
	t.Run("outputs same as reference implementation", func(t *testing.T) {
		...
		encoded, _, err := bitvector.Encode(expectedNumbers)
		assert.NoError(t, err)
		...
		decoded, err := bitvector.Decode(referenceEncoding)
		assert.NoError(t, err)
	...
	t.Run("RunLengths", func(t *testing.T) {
		...
		for _, testCase := range testCases {
			first, runs := bitvector.RunLengths(testCase.ints)
