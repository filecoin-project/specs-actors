...
func AdvanceOneEpoch(t *testing.T, v *VM) *VM {
...
func CheckSectorHealth(t *testing.T, v *VM, minerIDAddress address.Address, deadlineIndex uint64, partitionIndex uint64, sectorNumber abi.SectorNumber) bool {
	var minerState miner.State
	err := v.GetState(minerIDAddress, &minerState)
	require.NoError(t, err)

	return minerState.CheckSectorHealth(v.Store(), deadlineIndex, partitionIndex, sectorNumber) == nil
}

// returns true if the sector is faulty -- a slightly more specific check than CheckSectorHealth
